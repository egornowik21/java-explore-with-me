...
public interface StatRepostitory extends JpaRepository<Stat, Long> {
    ...
    @Query(value = "select new ru.yandex.practicum.statsserver.dto.ViewStatDto(st.app, st.uri,count(st.uri) as hits) " +
            "from Stat as st " +
    ...
    @Query(value = "select new ru.yandex.practicum.statsserver.dto.ViewStatDto(st.app, st.uri,count(DISTINCT st.uri) as hits) " +
            "from Stat as st " +
            "where timestamp between ?1 and ?2 " +
            "and st.uri in ?3 " +
            "group by st.app, st.uri " +
